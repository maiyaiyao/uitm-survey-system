...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($sub_req_name) || empty($sec_ID)) throw new Exception("All fields are required.");

        $sql = "UPDATE sub_req SET sub_req_name = :name, sec_ID = :sec WHERE sub_req_ID = :id";
        $db->query($sql, [':name' => $sub_req_name, ':sec' => $sec_ID, ':id' => $id]);
        
        setFlashMessage('success', "Requirement updated successfully.");
        header('Location: index.php?tab=requirements');
        exit;
    } catch (Exception $e) {
...
<head>
    <meta charset="UTF-8">
    <title>Edit Requirement - <?php echo APP_NAME; ?></title>
                ...
                <div class="main-content px-4 py-4">
                    <nav aria-label="breadcrumb" class="mb-4">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active text-dark">Edit Requirement</li>
                        </ol>
                    </nav>

                    <div class="card border-0 shadow-sm rounded-4">
                        <div class="card-header bg-white border-bottom py-3">
                            <h5 class="mb-0 fw-bold">Edit Requirement: <?php echo htmlspecialchars($id ?? ''); ?></h5>
                        </div>
                        <div class="card-body p-4">
                            ...
                            <form method="POST">
                                    ...
                                    <select name="sec_ID" class="form-select" required>
                                        <?php foreach ($sections as $sec): ?>
                                            <option value="<?php echo htmlspecialchars($sec['sec_ID']); ?>" 
                                                <?php echo ($sec['sec_ID'] == ($req['sec_ID'] ?? '')) ? 'selected' : ''; ?>>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="sub_req_name" class="form-control" rows="3" required><?php echo htmlspecialchars($req['sub_req_name'] ?? ''); ?></textarea>
                                </div>
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="index.php?tab=requirements" class="btn btn-light">Cancel</a>
                                    <button type="submit" class="btn btn-gradient-primary px-4">Update Requirement</button>
                                </div>
                            </form>
                        </div>
                    </div>
